...
ClapTrap::ClapTrap(std::string name, int energy, int m_energy, int melee_d, int range_d, int armor_reduc) : _name(name), _energy(energy), _max_energy(m_energy), _melee_dmg(melee_d), _ranged_dmg(range_d), _armor_reduc(armor_reduc) {
	this->_hp = 100;
	this->_max_hp = 100;
	this->_level = 1;
...
ClapTrap::~ClapTrap() {
	std::cout << "CL4P-TP - Destroyed : " << this->_name << std::endl;
}
